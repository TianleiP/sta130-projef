...
sim_tibble
ggplot(data = sim_tibble, aes(x = value)) + geom_histogram()
...
cah2 <- "STA130_APOGEE.h5" %>% h5read("ca_h") %>% as_tibble() 
cah_abundance <- cah2 %>% mutate(abundance = case_when(value < 0 ~ 0, value >= 0 ~ 1))
...
df4 = data.frame(loggra, cah = cah2$value, alh = alh$value, mgh = mgh$value, feh = feh$value)
