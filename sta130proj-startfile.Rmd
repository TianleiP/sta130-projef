...
model3 <- lm(loggra ~ feh_abundance$abundance + cah_abundance$abundance +mgh_abundance$abundance +  alh_abundance$abundance, data = df) %>% summary
model3

df3 <- data.frame(loggra, ca_al_abundance_interact = cah$value + alh$value, cah = cah$value, alh = alh$value)

df3 %>% ggplot(aes(x = loggra, y = ca_al_abundance_interact)) +
  geom_point() +
  geom_smooth(method = "lm")
# 
# df4 <- data.frame(loggra, ca_abundance = cah_abundance$abundance)
# df4
# df4 %>% ggplot(aes(x = ca_abundance, y = loggra)) +
#   geom_point() +
#   geom_smooth(method = "lm")
